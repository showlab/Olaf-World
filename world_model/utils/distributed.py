...
def fsdp_wrap(
    ...
    fsdp_kwargs = dict(
        ...
        sync_module_states=False,  # Keep original behavior
    )
    if ignored_modules:
        fsdp_kwargs["ignored_modules"] = ignored_modules

    # Some torch versions may not support ignored_modules; fall back safely.
    try:
        module = FSDP(module, **fsdp_kwargs)
    except TypeError:
        fsdp_kwargs.pop("ignored_modules", None)
        module = FSDP(module, **fsdp_kwargs)
    return module
