...
class ActionODERegressionLMDBDataset(Dataset):
    ...
    LMDB schema (per sample):
      first_frame_{i}_data / _shape / _dtype
      actions_{i}_data     / _shape / _dtype
      trajectories_{i}_data/ _shape / _dtype
    Global:
      length

    Returns:
      {
        "first_frame":  torch.FloatTensor [1,3,H,W]
        "actions":      torch.FloatTensor [T,A]
        "ode_latent":   torch.FloatTensor [K,F,16,H8,W8]
      }
    All outputs are explicitly float32.
    """

    def __init__(self, lmdb_path: str, max_len: int | None = None):
        self.env = lmdb.open(
            lmdb_path,
            readonly=True,
            lock=False,
        ...
        if max_len is not None:
            self.length = min(self.length, int(max_len))
    ...
    def _get_length(self) -> int:
        with self.env.begin() as txn:
            ...
            if val is None:
                raise KeyError("LMDB is missing global key 'length'")
            return int(val.decode())

    def _fetch_array(self, name: str, idx: int) -> np.ndarray:
        sk = f"{name}_{idx}_shape".encode()
        dk = f"{name}_{idx}_dtype".encode()
        bk = f"{name}_{idx}_data".encode()
        with self.env.begin() as txn:
            shape_b = txn.get(sk)
            dtype_b = txn.get(dk)
            data_b  = txn.get(bk)
        if shape_b is None or dtype_b is None or data_b is None:
            raise KeyError(f"Missing LMDB entries for {name}[{idx}]")

        shape = tuple(map(int, shape_b.decode().split()))
        dtype = np.dtype(dtype_b.decode())
        arr = np.frombuffer(data_b, dtype=dtype).reshape(shape)

        # 🔒 Double check: force float32
    ...
    def __getitem__(self, idx: int):
        y_cond = self._fetch_array("y_cond", idx)  # [16,25,68,120]
        clip_feats = self._fetch_array("clip_feats", idx)  # [3,H,W]
        acts = self._fetch_array("actions",      idx)   # [T,A]
        traj = self._fetch_array("trajectories", idx)   # [K,F,16,H8,W8]

        y_cond_t = torch.tensor(y_cond)
        clip_feats_t = torch.tensor(clip_feats)
