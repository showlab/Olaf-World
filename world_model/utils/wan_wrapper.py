...
class WanDiffusionWrapper(torch.nn.Module):
    ...
    def forward(
        ...
        if self.uniform_timestep:
            # Detect BI I2V: frame-0 timestep is all zeros, other frames share the same t
            if timestep.ndim == 2 and timestep.shape[1] > 1:
                ...
                # print(f"Detected BI I2V: {is_i2v}")
                input_timestep = timestep[:, 1] if is_i2v else timestep[:, 0]
                # print(f"Using timestep: {input_timestep}")
            else:
                # normal bidirectional diffusion
